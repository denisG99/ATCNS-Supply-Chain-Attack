...
class GitLogParser:
    ...
    def __init__(self, gitlog: str=""):
        ...
        """
        if not gitlog == "":
            self.__log: dict = self.__parse(gitlog)
        else:
    ...
    def __parse(self, gitlog: str) -> dict:
        ...
        for commit in commits:
            ...
            diffs = re.split(r'@@\s+-\d+(?:,\d+)?\s+\+\d+(?:,\d+)?\s+@@', commit)

            commits_info[hash] = {"author": author,
                                  "mail": mail,
                                  "datetime": datetime,
                                  "description": description.group(1) if description is not None else "",
                                  "diffs": diffs[1] if len(diffs) > 1 else ""}

        return commits_info

    def get_commits_hashes(self) -> list:
        return list(self.__log.keys())

    def get_commit_by_hash(self, hash: str) -> dict:
        return self.__log[hash]

    def get_commit_author(self, hash: str) -> str:
        return self.__log[hash]["author"]

    def get_commit_mail(self, hash: str) -> str:
        return self.__log[hash]["mail"]

    def get_commit_datetime(self, hash: str) -> str:
        return self.__log[hash]["datetime"]

    def get_commit_description(self, hash: str) -> str:
        return self.__log[hash]["description"]

    def get_commit_diffs(self, hash: str) -> str:
        return self.__log[hash]["diffs"]

    def gitlog_is_empty(self) -> bool:
        """
        Check if the git log is empty

        :return: True if the git log is empty, False otherwise
        """
        return not bool(self.__log)